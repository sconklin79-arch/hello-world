...
## Layout
- `utils.py` — small standalone helper functions (`average`, `get_first`, `parse_int`, `divide`), each guarded against empty/invalid/zero-divisor input.
- `test_utils.py` — unit tests for `utils.py`, run by CI via `unittest discover`.
